...
At an intuitive level, the relocation mechanism repeats the following loop. The system first identifies which user snapshots are geographically close to each candidate wall-mounted site. It then uses the channel quality measured for those local user snapshots to decide which candidate sites are most promising, and finally reassigns the movable \glspl{ap} to a new subset of locations. Recent channel observations are given more importance than older ones, so the controller reacts to evolving user concentrations without discarding past information entirely. Hence, position information determines which samples are considered local, whereas the actual ranking is driven by the corresponding \gls{csi}-derived link quality.
...
\end{equation}
This restriction makes the relocation rule sensitive to the CSI observed near the candidate sites rather than to globally averaged scene statistics. In other words, user positions are used to construct the local sampling mask, but they do not enter the score directly.
