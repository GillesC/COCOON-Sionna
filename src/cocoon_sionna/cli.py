...
def _default_log_path(command: str, scenario_path: str, config) -> Path | None:
    ...
    if command == "all":
        return config.outputs.output_dir / "all.log"
...
def _postprocess_payload(args, output_dir: Path) -> dict[str, Path]:
    return run_visualization_postprocess(
...
def main() -> None:
    ...
    all_parser = subparsers.add_parser("all", help="Run a full scenario and then rebuild all visualizations and analyses")
    all_parser.add_argument("scenario", help="Path to a scenario YAML file")
    all_parser.add_argument("--analysis-dir", type=Path, default=None, help="Directory where the postprocessing files should be written")
    all_parser.add_argument("--threshold-min-db", type=float, default=-10.0)
    all_parser.add_argument("--threshold-max-db", type=float, default=20.0)
    all_parser.add_argument("--threshold-step-db", type=float, default=1.0)
    all_parser.add_argument("--outage-threshold-db", type=float, default=0.0)
    ...
    if args.command == "postprocess":
        ...
        print(
            json.dumps(
                _postprocess_payload(args, output_dir),
                indent=2,
                default=str,
            )
        )
    ...
    if args.command == "run":
        ...
        run_payload = run_scenario(config)
        print(
            json.dumps(
                {
                    "run": run_payload,
                    "postprocess": _postprocess_payload(args, config.outputs.output_dir),
                },
                indent=2,
                default=str,
            )
