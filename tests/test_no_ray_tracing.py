...
def test_run_scenario_without_ray_tracing_writes_trajectory_outputs(monkeypatch, tmp_path: Path):
    ...
    assert (config.outputs.output_dir / "distributed_movable_optimization_3_aps.csv").exists()
    assert (config.outputs.output_dir / "distributed_movable_optimization_3_schedule.csv").exists()
